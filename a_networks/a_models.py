...
from keras.layers import SeparableConv2D, Conv2D, LeakyReLU, UpSampling2D, ReLU, MaxPooling2D
from keras.applications.vgg19 import VGG19, preprocess_input
from tensorflow_addons.layers import SpectralNormalization
from a_networks.a_blocks import ARRDB
...
class ARRDBNet(Model):
  def __init__(self, input_shape):
    ...
    self.rb_5 = ARRDB()
    self.rb_6 = ARRDB()
    self.conv_body = SeparableConv2D(64, 3, strides=(1, 1), padding="SAME") # padding 1

    self.up_1 = UpSampling2D()
    self.conv_1 = SeparableConv2D(64, 3, strides=(1, 1), padding="SAME") # padding 1
    self.activation_1 = LeakyReLU(alpha=0.2)

    self.up_2 = UpSampling2D()
    self.conv_2 = SeparableConv2D(64, 3, strides=(1, 1), padding="SAME") # padding 1
    self.activation_2 = LeakyReLU(alpha=0.2)

    self.conv_3 = SeparableConv2D(64, 3, strides=(1, 1), padding="SAME") # padding 1
    self.activation_3 = LeakyReLU(alpha=0.2)
    self.conv_4 = SeparableConv2D(3, 3, strides=(1, 1), padding="SAME") # padding 1
